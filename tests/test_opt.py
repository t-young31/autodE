...
def test_inverse_dist_prim():
    ...
    primitives = internals.PIC()

    for i in range(methane.n_atoms):
        for j in range(i+1, methane.n_atoms):
            primitives.append(internals.InverseDistance(idx_i=i, idx_j=j))

    assert np.isclose(primitives.q(x)[0],
...
def test_inverse_dist_dic_methane():
    ...
    x_mat = coords.x.reshape((-1, 3))

    assert coords.x.shape == (3*methane.n_atoms,)

    n_expected = methane.n_atoms * (methane.n_atoms - 1) // 2
    assert len(coords.primitives) == n_expected
    B_q = coords.primitives.B(coords.x)
    assert B_q.shape == (n_expected, 3 * methane.n_atoms)

    # ---------- check some elements of the B matrix ----------------
    assert np.isclose(B_q[0, 0],
                      coords.primitives[0].derivative(0, 0, x=x_mat))

    # dq_2/dx_(0,0) is zero as the 0-th element doesn't appear in that distance
    assert np.isclose(B_q[-1, 0], 0)

    assert np.isclose(B_q[1, 3],
                      coords.primitives[1].derivative(i=1, k=0, x=x_mat))
    ...
    assert 0.4 < np.average(coords.primitives.q(coords.x)) < 1.0
