import pytest
import numpy as np
from .molecules import h2_mol, methane_mol, water_mol, h2o2_mol
...
from autode.opt.coordinates.cartesian import (CartesianCoordinates3D,
                                              ...
                                              _rotation_matrix_into_xy_plane)
...
def test_cartesian_coordinates_non_planar():

    x = cartesian_coordinates(methane_mol())
    assert x.shape == (5, 3)  # 3D coordinates


def test_cartesian_coordinates_planar():

    water = water_mol()
    water.rotate(axis=[0.1, 0.2, 0.3], theta=0.4)
    assert water.is_planar()

    init_coords = water.coordinates
    assert not np.allclose(init_coords[:, 2],
                           np.zeros(shape=(3,)))

    x = cartesian_coordinates(water)
    assert x.shape == (3, 2)  # 2D coordinates

    # Transformation should preserve distances
    for (i, j) in ((0, 1), (0, 2), (1, 2)):
        assert np.isclose(np.linalg.norm(x[i, :] - x[j, :]),
                          water.distance(i, j))

    assert np.allclose(x.to_3d(), init_coords)


def test_water_b_matrix_planar():

    x = cartesian_coordinates(water_mol())
    pic = InverseDistances.from_cartesian(x)
    _ = pic(x)

    assert len(pic) == 3          # 3 primitives
    assert pic.B.shape == (3, 2*3)  # with dq/dx dq/dy only for each atom


def test_methane_b_matrix():

    methane = methane_mol()
    x = cartesian_coordinates(methane)
    pic = InverseDistances.from_cartesian(x)
    _ = pic(x)

    assert pic.B.shape[1] == 3 * methane.n_atoms  # has x, y, z components


def test_rotation_into_xy_plane():

    h4 = Molecule(atoms=[Atom("H", x=-1.),
                         Atom("H", x=1.),
                         Atom("H", y=1.),
                         Atom("H", y=-1.)])
    init_coords = h4.coordinates

    def is_in_xy_plane(arr):
        """All the z coordinates should be 0"""
        return np.isclose(np.sum(np.abs(arr[:, 2])), 0.)

    assert is_in_xy_plane(h4.coordinates)

    assert np.allclose(_rotation_matrix_into_xy_plane(h4.atoms),
                       np.eye(3))

    for _ in range(10):
        h4.coordinates = init_coords

        # Rotate and translate randomly
        h4.rotate(axis=np.random.random(3),
                  theta=np.random.random())
        h4.translate(np.random.random(3))
        h4.translate(-h4.atoms[0].coord)

        new_coords = np.array(h4.coordinates).dot(
            _rotation_matrix_into_xy_plane(h4.atoms).T
        )

        assert is_in_xy_plane(new_coords)


def test_dic_from_2d_cartesian():

    x = cartesian_coordinates(water_mol())
    dic = DIC.from_cartesian(x)

    # Should have 3N-6 DOF, where N_atoms = 3 for water
    assert len(dic) == 3 * 3 - 6

    # and can back transform to cartesians
    assert np.allclose(x, dic.to("cartesian"))

    # and should be able to shift in internal coordinates
    dic += np.array([0.01, 0.02, 0.03])
...
def test_primitives_equality():
    ...
    x = CartesianCoordinates3D(h2_mol().coordinates)
...
def test_cartesian_coordinates():
    ...
    x = CartesianCoordinates3D(arr)  # defaults to Å
    ...
    assert 0.19 < CartesianCoordinates3D(arr).to('nm')[1][0] < 0.21

    # but not an unsupported unit
    with pytest.raises(ValueError):
        _ = CartesianCoordinates3D(arr).to('X')


def test_cartesian_coordinates_hessian_update():

    # Simple coordinates with 2 atoms in 3 D
    coords = CartesianCoordinates3D(np.arange(0, 6).reshape((2, 3)))
...
def test_cartesian_coordinate_shift_type():
    ...
    coords = CartesianCoordinates3D(np.array([0.0]))

    # Shifting coordinates should retain the type
    assert isinstance(coords - 0.1, CartesianCoordinates3D)
    assert isinstance(0.1 - coords, CartesianCoordinates3D)
    assert isinstance(coords + 0.1, CartesianCoordinates3D)
    assert isinstance(0.1 + coords, CartesianCoordinates3D)
...
def test_hessian_set():
    ...
    coords = CartesianCoordinates3D(np.array([1.0, 2.0]))
...
def test_cartesian_update_clear():
    ...
    x += np.arange(0, 6, dtype=float).reshape((2, 3))
    assert x.g is None and x.h is None
...
def test_cart_to_dic():
    ...
    x += 0.1
    assert np.allclose(x,  np.array([[0.1, 0.1, 0.1],
                                     [2.1, 0.1, 0.1]]))
...
def test_simple_dic_to_cart():
    ...
    dic = CartesianCoordinates3D(arr).to('dic')

    x = dic.to('cartesian')
    assert np.allclose(CartesianCoordinates3D(arr), x)
...
def test_methane_cart_to_dic():
    ...
    x = CartesianCoordinates3D(methane_mol().coordinates)
    dic = x.to('dic')
...
def test_grad_transform_linear():
    ...
    def num_grad(_x, h=1E-8):
        ...
        for i in range(len(_x.flatten())):
            ...
            x_ph = np.array(_x, copy=True).flatten()
            x_ph[i] += h
    ...
    x = CartesianCoordinates3D(coords).flatten()
...
def test_hess_positive_definite_h2o():
    ...
    coords = CartesianCoordinates3D(coords_arr)
...
def test_inplace_subtraction():
    ...
    coords = CartesianCoordinates3D(np.array([1.0]))
...
def test_coords_back_transform_tensor_clear():
    ...
    cart = CartesianCoordinates3D(raw_arr)
