...
class TestBFGSOptimiser2D(BFGSOptimiser):
    ...
    def __init__(self, e_func, g_func, init_x, init_y,
                 maxiter=30, etol=1E-4, gtol=1E-3,
                 line_search_type=NullLineSearch, init_alpha=1.0):
        super().__init__(maxiter=maxiter, line_search_type=line_search_type,
...
class TestBFGSOptimiser(TestBFGSOptimiser2D):
    ...
    def __init__(self, **kwargs):
        super().__init__(e_func=lambda x, y: x**2 + y**2,
                         g_func=lambda x, y: np.array([2.0*x, 2.0*y]),
                         init_y=1.0, init_x=1.0, **kwargs)
...
def check_gaussian_well_opt(init_x, init_y):
    ...
    assert np.allclose(optimiser._coords.h,
                       hessian(*optimiser._coords),
                       atol=1)
...
def test_complex_2d_opt():
    ...
    def grad(x, y):
        return np.array([2 * (20 * x ** 3 - 20 * x * y + x - 1),
                         20 * (y - x ** 2)])

    optimiser = TestBFGSOptimiser2D(e_func=energy, g_func=grad,
                                    init_x=-1.0, init_y=1.0,
                                    init_alpha=0.1, maxiter=100)
    optimiser.run(Molecule(name='blank'), method=Method())
